...
Later same day: `SignInGate` removed — the app must never fully block, only ever show `ReauthBanner`. Sign-out moved behind a Cancel/confirm modal (was instant). `ResearchPage.tsx` fully recolored from its original violet/sky/rose/emerald multi-color scheme to the single teal identity used by `TransactionsPage`/`HoldingsPage`/`PortfoliosPage` — no more per-tab color coding. Its Nav row + Overview card fused into one visually unified block (nav row `border-4 rounded-t-[14px]`, card `border-4 border-t-0 rounded-b-[14px]`, no gap), matching the Transactions page's Nav-bar-then-SummaryCard pattern exactly, including reusing `SummaryCard`'s dark-hero gradient look (`linear-gradient(150deg,#10243f,#0b3b3a)`, radial glow, `text-[8px]` uppercase stat labels) for the Overview card content (price/CAGR/52W instead of invested/gain/XIRR, since Explore isn't tied to a holding). New top-right Settings popover on `ResearchPage.tsx` standardized to the same 320px/`px-4 py-[11px]` header/`text-[12px] font-bold` row sizing already used on Holdings/Transactions/Portfolios Settings modals — the Transactions page's own modal had drifted smaller (`300px`, `px-2.5 py-1.5`) and was widened back to match in the same pass. Deep Research full-screen reading toggle (Research/Transactions/Holdings pages) collapses the header down to just the sub-tab strip; implemented via a CSS var (`--reauth-banner-h`) the banner publishes so height-locked pages can reserve space for it instead of it overlapping their nav.

Later still same day: `ResearchPage.tsx`'s outer header/content padding was `px-4 pt-4` — doubled up against the Nav row's own internal `px-4`, making the whole page read as over-padded next to Transactions page. Matched to Transactions page's `px-1` (no separate top padding), which also fixed the same visual excess in the Quick Stats grid/Deep Research cards since they render inside that container. Deep Research cards: opening one now collapses all others (accordion, restoring pre-2026-06 behavior) for manual taps only — `handleGenerate`'s auto-expand-on-new-generation deliberately stays independent per a past session's fix for a "jarring collapse mid-read" bug; double-click/tap anywhere in an expanded card's response now collapses it. `ReauthBanner` redesigned again: swapped amber for the app's teal palette, text left-aligned/button right-aligned, added a dismiss (×) and a 5s auto-dismiss (paused while the sign-in panel is open), and clicking "Sign in" now closes the banner and opens a separate centered/blurred-backdrop modal (matching the existing Settings→Account sign-in modal pattern) instead of expanding inline within the banner.
...
Valuation card prompt: removed the DCF/Gordon-growth/EV-multiples model-selection section (was giving wrong outputs); refocused "Growth & Price Trend Analysis" around a 1yr/3yr/5yr revenue+income CAGR vs price CAGR table, added 52-week low alongside the existing high. Multiples Snapshot (PE historical + peer/sector PE) and the rest of the card's sections unchanged.

New: Price Alerts + push notifications. Bell icon (unread-count badge) on Overview page header, opens a bottom-sheet list — unread rows `bg-teal-50 border-l-4 border-teal-500` + bold, read rows plain white/slate-50, tap navigates to the holding + marks read, small `×` dismisses. `ManageAlertsModal` (opened via a new 🔔 icon on TransactionsPage) redesigned mid-session: two independent rows — **% Move** and **Price Level** — each with a compact `w-7 h-7` ↓/↑ direction-toggle button inline with its own threshold input (not a big always-visible segmented control), label-left/controls-right matching the existing toggle-row pattern (Dividends/FX/Push toggles). Both rows fillable at once — submitting creates one rule per filled row, so an alert fires the moment either condition is met. Existing-alerts list kept compact (`p-2`, `py-1` action links) per explicit request. Global "Push notifications" toggle added to the Settings popover (Configuration section, same switch styling as Dividends/FX toggles). Later same day, bell's bottom sheet gained a "Show all set alerts" footer button toggling to a compact all-rules view (name + condition + Active/Triggered/Disabled badge), tap deep-links into that holding's `ManageAlertsModal`.

New: Watchlist. Star (☆/★ amber-filled) on the Research page overview card next to the company name, no separate confirmation — tap toggles immediately. Lives in the existing Explore FAB modal on the Overview page, not a new route. Explore modal reworked to drop the Search/Watchlist tab toggle and Recent-searches chips entirely: single view, search bar directly under the header, live Results while typing else the Watchlist list underneath (avatar + name + live price + today's %, matching the search-result row's visual language). Removing a row is a right-swipe (red "Remove" revealed underneath, ~72px threshold) instead of a small ✕ button — reads better once rows show live numbers on the right, no separate tap target needed. Modal anchor changed from vertically-centered to `items-start pt-6` so a longer watchlist has room to show without feeling cramped.

Debug Log (🐛 diagnostic panel) moved out of its own floating bottom-left button — now a "Debug Log" row inside Settings → Configuration on the Overview page, next to Manage Buckets, same row style (icon-button trailing a two-line label).

**Later same day — Settings modal full rework + Nexus rebrand.** App renamed Stock Analyzer/Portfolio Manager → Nexus everywhere (tab title, PWA manifest, in-app loading/sign-in screens, `/join`); live domain migrated `stock-analyzer-blush.vercel.app` → `nexusconfig.vercel.app` (old domain fully retired from CORS + docs). Settings modal reordered to Account → Data → Configuration (was Data → Account → Config); Configuration is now collapsible (chevron header, default collapsed) to cut visual clutter for signed-out users. Account section rebuilt as three same-style rows (`bg-emerald-50/60` cards, matching Data's style instead of the old distinct `bg-slate-50` treatment): "Sign in with Google" (tapping now opens a small centered modal with backdrop instead of expanding inline in the row), "Import CSV" (moved here from Data), "Backup to Drive" (shows "Last backup: <timestamp>" inline) and a new separate "Restore from Drive" row — two-step: first tap checks Drive and shows the backup's timestamp without touching local data, second tap on the same button actually restores (avoids one-tap accidental overwrite). New "Admin panel" section (admin-only): "Manage Users" row opens a dedicated modal (same visual pattern as Manage Buckets) holding the add/list/revoke/restore/delete UI that used to be crammed inline; "Debug Log" row sits beside it (no longer visible to non-admins). Footer "Updated on" row given a `bg-slate-100` card background instead of a bare border-top. Import-fail banner (top-of-screen, shown on failed CSV import) widened to `w-[92vw] max-w-sm` and changed `rounded-full` → `rounded-xl` since the pill shape broke on 3-line wraps; duration extended 2-2.5s → 5s across all failure cases.
