...
Overview Settings: the "Updated on" (build version/timestamp) row moved out of the always-visible footer and into the admin-only Admin panel section (as a third static row alongside Manage Users/Debug Log, same `bg-slate-100` styling as before) — regular users no longer see build/version info at all.
