...
| 2026-05-31 | Allocation tab makeover — replaced 3 nested accordions (By Sector, By Market Cap, Concentration) with 3-pill view toggle + `@nivo/sunburst` donut chart; `AllocNodeDatum` interface; `sunburstSectorData` + `sunburstMktCapData` useMemos; `allocView` + `selectedAllocSector` state (removed `expandedAllocSectors`, `expandedMktCapBuckets`, `sectorSectionOpen`, `mktCapSectionOpen`, `concentrationSectionOpen`); inner ring = sectors/buckets, outer ring = individual holdings (translucent); center label shows total value or selected sector stats+XIRR; tap sector arc → filter + info strip; tap holding arc → navigate to TransactionsPage; flat `shadow-sm` holding cards replace rows; Concentration view retained |
