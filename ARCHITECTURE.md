...
frontend/
  src/
    ...
    hooks/useDividends.ts        LS_KEY: global (portfolio='') → "dividends:cache:" (preserved); per-portfolio → "dividends:cache:v2:{p}" (v2 busts stale wrong-data entries where per-portfolio views showed full 1.75L instead of scoped amount); staleTime=30d; placeholderData from localStorage; clearDividendLocalCache() wipes all "dividends:cache:*" keys (called on CSV upload)
...
| price history (daily) | market-aware via `market_hours.is_stale()` — open market: 30min; closed market: until next exchange close (not a flat TTL) |
| price history (intraday 1d) | 1hr |
