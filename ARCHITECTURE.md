...
backend/
  ...
  routers/
    ...
    history.py              GET /api/history?yf_symbol=INFY.NS&start=YYYY-MM-DD&since=YYYY-MM-DD OR ?period=1d (intraday; timestamps in IST; includes prev_close); daily history cached as one contiguous per-symbol series in `_series_cache` (not month-bucketed); `_fetch_incremental()` re-fetches only from the last cached bar forward and merges, instead of re-downloading the full 2015→now range every call; staleness gated by `market_hours.is_stale()` instead of a flat TTL; `_series_cache`/`_intraday_cache` capped to the 300 most-recently-fetched symbols each (`_evict_oldest()`) — were unbounded (every symbol ever requested across every Explore/Quick-Stats lookup, never evicted), the likely cause of Render's memory-limit restarts; intraday fetch (`_fetch_intraday`) runs the 5m-bars download and the prev_close lookup concurrently via `asyncio.gather` instead of sequentially, and prev_close now uses `price_fetcher.get_prices_and_prev_close()` (lightweight quote) instead of a second full OHLCV download; `_sem` concurrency cap is 4 — briefly raised to 8 to clear mass-refetch bursts faster, but each concurrent `yf.download()` holds a full OHLCV DataFrame in memory and 8-at-once pinned Render's 512MB free-tier instance at its OOM ceiling (~15.4MB/call observed live); reverted to 4. `_download()`/`_fetch_intraday_bars()` now `del df` immediately after extracting the `Close` series (`.copy()`'d first so it's detached from the parent frame's memory) instead of holding the full OHLCV frame until function return. `_series_cache` write-through persists to disk via `src/cache.py`'s `Cache` (`hist:{symbol}` prefix) on every write (`_save_entry()`) — survives an OOM-restart or redeploy within the same container, so the next request for that symbol does a delta fetch instead of a full 2015→now re-download (verified: 1.6s cold fetch → 0.3s after a simulated restart). `since` query param (optional) lets the client hint its own already-cached last date — when the *server's* cache (memory + disk) is genuinely cold, `_fetch_incremental()` does a delta-only fetch from that date and returns it tagged `partial_since` instead of a full range; deliberately **not** written into the shared cache (a `since` hint is per-browser, not representative of what other callers already have) — frontend merges the delta into its own localStorage rather than replacing it (`mergeHistory()` in `useHistory.ts`)
