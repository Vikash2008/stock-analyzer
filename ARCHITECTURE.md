...
frontend/
  src/
    ...
    hooks/useDividends.ts        useDividends(portfolio?) — TanStack Query ['dividends', portfolio ?? ''], staleTime=24h, gcTime=Infinity; useForceRefreshDividends(portfolio?); useDividendForSymbol(symbol) — reads from ['dividends', ''] global cache (no portfolio filter); getIncludeDividends()/setIncludeDividends() — localStorage 'settings.includeDividends'
