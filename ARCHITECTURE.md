...
frontend/
  src/
    ...
    api/gemini.ts                fetchGeminiSection(symbol, sectionId, prompt, forceRefresh?, forceLite?, keyIndex?) → GeminiResponse {text, sources[], grounded, model}; fetchGeminiChat(symbol, question, contextText, forceLite?, keyIndex?) → GeminiResponse
    api/dividends.ts             DividendEvent / DividendSymbol / DividendsData interfaces; fetchDividends(forceRefresh?) → DividendsData
    hooks/useDividends.ts        useDividends() — TanStack Query ['dividends'], staleTime=24h, gcTime=Infinity; useForceRefreshDividends(); useDividendForSymbol(symbol) — reads from query cache without API call; getIncludeDividends()/setIncludeDividends() — localStorage 'settings.includeDividends'
...
| GET | `/api/dividends` | `force_refresh=false` | Auto-fetched dividend history for all held stocks; 24h in-memory cache; reads from FIFO cache (user's uploaded CSV) |
