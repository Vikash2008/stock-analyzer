...
| **Login / Access Control (NEW 2026-08-13)** | Google Sign-In via `frontend/src/components/GoogleSignInButton.tsx`; session in `frontend/src/utils/auth.ts` (idbStore-backed JWT); `App.tsx`'s `SignInGate` (no cached data, blocking) / `ReauthBanner` (stale data shown, non-blocking, centered/amber, "Sign in" button hides once panel opens) surface it; Settings → Account section in `PortfoliosPage.tsx` — reworked same day into 3 rows (Sign in with Google, Import CSV, Backup to Drive); tapping "Sign in with Google" opens a small centered modal (backdrop, own header) instead of expanding inline; `frontend/src/pages/JoinPage.tsx` (`/join`, plain landing page, no token) | `backend/routers/auth.py` (`POST /api/auth/google`, `GET /api/auth/me`), `backend/users_store.py` (`data/.users.json`), `backend/auth_deps.py` (`get_current_user`/`get_current_admin` — admin status is the static `ADMIN_EMAILS` env var, separate from users_store) |
