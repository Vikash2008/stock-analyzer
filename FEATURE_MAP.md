...
| **Price Alerts / Push Notifications (NEW 2026-08-11)** | `frontend/src/components/NotificationBell.tsx` + `NotificationsPanel.tsx` (bell icon + unread badge on `PortfoliosPage.tsx`, bottom-sheet list), `frontend/src/components/ManageAlertsModal.tsx` (opened from `TransactionsPage.tsx`'s 🔔 icon; two independent rows — % Move / Price Level — each fillable on its own), `frontend/src/hooks/useAlerts.ts`, `frontend/src/api/alerts.ts`, `frontend/src/utils/clientId.ts` (per-browser identity, no login system), `frontend/src/utils/pushSubscribe.ts`, `frontend/src/sw.ts` (custom service worker, push handling) | `backend/routers/alerts.py`, `backend/alerts_store.py` (client-partitioned JSON store), `backend/alerts_engine.py` (`evaluate()`, called from `backend/price_refresh.py`'s 2-min loop) |
...
| Alerts store (backend, disk) | `backend/alerts_store.py` (`data/.alerts.json`) | NEW 2026-08-11 — not a cache, user-authored rules; partitioned by `client_id` (`frontend/src/utils/clientId.ts`'s per-browser UUID, localStorage key `alerts:client_id`). Gitignored like `.cache.pkl`. |
