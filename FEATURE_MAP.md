...
| **Login / Access Control (NEW 2026-08-13)** | Google Sign-In via `frontend/src/components/GoogleSignInButton.tsx`, session in `frontend/src/utils/auth.ts` (idbStore-backed JWT); `App.tsx`'s `SignInGate` (no cached data, blocking) / `ReauthBanner` (stale data shown, non-blocking) surface it; Settings → Account row in `PortfoliosPage.tsx`; `frontend/src/pages/JoinPage.tsx` (`/join`, plain landing page, no token) | `backend/routers/auth.py` (`POST /api/auth/google`, `GET /api/auth/me`), `backend/users_store.py` (`data/.users.json`), `backend/auth_deps.py` (`get_current_user`/`get_current_admin`) |
| **Admin panel (NEW 2026-08-13)** | Settings → "Admin — Users" section in `PortfoliosPage.tsx` (only rendered if the users fetch succeeds); `frontend/src/hooks/useAdmin.ts`, `frontend/src/api/admin.ts` | `backend/routers/admin.py` — admin-only (`ADMIN_EMAILS` env var) add/revoke/restore/delete over `users_store.py` |
| **Google Drive backup (NEW 2026-08-13)** | Settings → Account: "Back up to Drive" / "Restore from Drive" in `PortfoliosPage.tsx`; `frontend/src/utils/driveBackup.ts` (`drive.file` OAuth scope, browser talks directly to Google's Drive API) | — (no backend involvement by design — see Pending/ARCHITECTURE.md) |
