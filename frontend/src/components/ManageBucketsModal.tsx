...
export function ManageBucketsModal({ open, onClose, data, onChanged }: Props) {
  ...
  // Sector has its own dedicated "Manage Category" modal — never show it in the generic list.
  const visibleBuckets = buckets.filter(b => b.name !== SECTOR_BUCKET)
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto space-y-2 px-2.5 py-2.5" style={{ background: '#f8fafc' }}>
          ...
          {/* Count + New bucket — Sector is managed in its own "Manage Category" modal */}
